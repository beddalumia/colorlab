...
%
% (c) 2013-2022 Stephen Cobeldick, 2022 Gabriele Bellomia (palette-package adaptation)
%
...
%
%%% Syntax:
%  map = palette.cubehelix;
%  map = palette.cubehelix(N);
%  map = palette.cubehelix(N,start,rots,satn,gamma);
%  map = palette.cubehelix(N,start,rots,satn,gamma,irange);
%  map = palette.cubehelix(N,start,rots,satn,gamma,irange,domain);
%  map = palette.cubehelix(N,[start,rots,satn,gamma],...)
%  map = palette.cubehelix([],...)
% [map,lo,hi] = palette.cubehelix(...)
%
...
%  palette.cubehelix(3, [0.5,-1.5,1,1], [0.2,0.8]) % irange=[0.2,0.8]
...
%  palette.cubehelix(3, [0.5,-1.5,1,1], [0.2,0.8], [0.3,0.7]) % domain=[0.3,0.7]
%  ans = 0.020144     0.29948      0.15693 % <- color, not gray shade
%        0.62751      0.47498      0.28642
%        0.91366      0.71351      0.95395 % <- color, not gray shade
%
% The function PALETTEDITOR.CUBEHELIX summons an ergonomic GUI to showreel
% all these options. If R>=2014b you can even feed to it the axes / figure
% handle and update interactively the plot itself.
%
%% Examples %%
%
...
% [X,Y,Z] = peaks(30);
% surfc(X,Y,Z)
% colormap(palette.cubehelix([],0.7,-0.7,2,1,[0.1,0.9],[0.1,0.9]))
% axis([-3,3,-3,3,-10,5])
%
...
%
% See also PALETTE PRESET_PALETTE PALETTEDITOR BREWER CRAMERI
