function [map,lo,hi,prm] = map(N,start,rots,satn,gamma,irange,domain)
...
%  map = cubehelix.map;
%  map = cubehelix.map(N);
%  map = cubehelix.map(N,start,rots,satn,gamma);
%  map = cubehelix.map(N,start,rots,satn,gamma,irange);
%  map = cubehelix.map(N,start,rots,satn,gamma,irange,domain);
%  map = cubehelix.map(N,[start,rots,satn,gamma],...)
%  map = cubehelix.map([],...)
% [map,lo,hi] = cubehelix.map(...)
%
...
%  cubehelix.map(3, [0.5,-1.5,1,1], [0.2,0.8]) % irange=[0.2,0.8]
...
%  cubehelix.map(3, [0.5,-1.5,1,1], [0.2,0.8], [0.3,0.7]) % domain=[0.3,0.7]
...
% >> S = load('spine');
% >> image(S.X)
% >> colormap(cubehelix.map)
%
%%% New colors for the SURF example:
% [X,Y,Z] = peaks(30);
% surfc(X,Y,Z)
% colormap(cubehelix.map([],0.7,-0.7,2,1,[0.1,0.9],[0.1,0.9]))
% axis([-3,3,-3,3,-10,5])
%
...
if nargin==0 || (isnumeric(N)&&isequal(N,[]))
	...
	assert(isnumeric(N)&&isscalar(N),...
		'SC:cubehelix.map:N:NotNumericScalar', err)
	assert(isreal(N)&&isfinite(N)&&fix(N)==N&&N>=0,...
		'SC:cubehelix.map:N:NotRealWhole', err)
	N = double(N);
end
%
...
elseif nargin<5
	...
	assert(isn(start,4)&&isvector(start),...
		'SC:cubehelix.map:start:NotVectorParameters',...
	...
	assert(iss(start), 'SC:cubehelix.map:start:NotNumericScalar', rsn,'start')
	assert(iss(rots),  'SC:cubehelix.map:rots:NotNumericScalar',  rsn,'rots')
	assert(iss(satn),  'SC:cubehelix.map:satn:NotNumericScalar',  rsn,'satn')
	assert(iss(gamma), 'SC:cubehelix.map:gamma:NotNumericScalar', rsn,'gamma')
	start = double(start);
	rots  = double(rots);
	satn  = double(satn);
	gamma = double(gamma);
...
if any(nargin==[0,1,2,5])
	...
	assert(isn(irange,2),...
		'SC:cubehelix.map:irange:NotNumericVector',...
...
if any(nargin==[0,1,2,3,5,6])
	...
	assert(isn(domain,2),...
		'SC:cubehelix.map:domain:NotNumericVector',...
