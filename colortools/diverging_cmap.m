...
%
% © Created by Andy Stein, released on Moreland's page under CC0 license.
% © Modified and embedded in COLORLAB by Gabriele Bellomia (MIT license).
%
% USAGE
%
...
%       - we definitely need better input checking, to come ASAP.
    ...
    if not(isnumeric(rgb1))
       rgb1 = str2rgb(rgb1);
    end
    
    if not(isnumeric(rgb2))
       rgb2 = str2rgb(rgb2);
    end
...
% Given 2 angular orientations, returns the smallest angle between the two
...
function[rgb] = XYZToRGB(xyz)
  ...
  % (since we can see colors outside of the display gamut), but this seems 
  % to work well enough.
  maxVal = r;
