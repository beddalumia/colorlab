function [map] = diverging_cmap(rgb1,rgb2,N)
% This function implements Kenneth Moreland's method for greating Diverging
% Colormaps that are perceptually uniform:
%
% > more info at https://www.kennethmoreland.com/color-maps/ 
%
% (c) Created by Andy Stein, released on Moreland's page under CC0 license.
% (c) Modified and embedded in COLORLAB by Gabriele Bellomia (MIT license).
%
% USAGE
%
%   >> [map] = diverging_map(rgb1,rgb2)
%
%   rgb1 : rgb triplet, the star point for your colormap.
%   rgb2 : rgb triplet, the end point for your colormap.
%   N    : integer, defining the number of color levels. [optional]
%
% TODO:
%       - we plan to support rgb1 and rgb2 to be passed also as strings,
%         exploiting the str2rgb function. Not implemented yet.
%       - we definitely need better input checking, to come ASAP.
%
% See also palette, preset_palette, str2rgb, rgb.X11, rgb.xkcd

    if nargin < 3 
       N = cmDefaultN(); % Default N same as MATLAB colormaps.
    end
    ...
    for i=1:length(s)
        map(i,:) = diverging_cmap_1val(s(i),rgb1,rgb2);
    end
...
% Interpolate a diverging color map.
    function[result] = diverging_cmap_1val(s,rgb1,rgb2)
        
        % s must be a number between 0 and 1
        assert(0 <= s && s <= 1,'divergin_cmap_1val requires normalized data')
